...
def eval(pc: Piece, m: Move, en: Int, b: Board) : Set[Piece] = {
  pc.pos match{
    ...
    case ps if en == 0 && is_occupied(ps, b) => if (occupied_by(ps, b).get != pc.col) {
      Set(pc)
    } else {
      Set()
    }
    ...
    case _ => m match {
      ...
      case RU => eval(incx(pc), m, en - 1, b).union(eval(pc, U, en, b))
      case LU => eval(decx(pc), m, en - 1, b).union(eval(pc, U, en, b))
      case RD => eval(incx(pc), m, en - 1, b).union(eval(pc, D, en, b))
      case LD => eval(decx(pc), m, en - 1, b).union(eval(pc, D, en, b))
      case UR => eval(incy(pc), m, en - 1, b).union(eval(pc, R, en, b))
      case UL => eval(incy(pc), m, en - 1, b).union(eval(pc, L, en, b))
      case DR => eval(decy(pc), m, en - 1, b).union(eval(pc, R, en, b))
      case DL => eval(decy(pc), m, en - 1, b).union(eval(pc, L, en, b))
    }
  }
}
...
def all_moves(pc: Piece, b: Board) : Set[Piece] = {
  val moves = List(U, D, L, R, RU, LU, RD, LD, UR, UL, DR, DL)
...
def attacked(c: Colour, b: Board) : Set[Piece] = {
  ...
  val pieces_colour = coloured.flatMap(piece => all_moves(piece, b)).map(_.pos).toSet
  opposites.filter(piece => pieces_colour.contains(piece.pos))
}
...
def legal_moves(pc: Piece, b: Board) : Set[Piece] = {
  pc match {
    ...
    case obj: King => all_moves(pc, b).map(piece => (piece, make_board(piece, b))).filter(tup => attackedN(tup._1, tup._2) == 0).map(_._1)
  }
}

def make_board(pc: Piece, b: Board) : Board = {
